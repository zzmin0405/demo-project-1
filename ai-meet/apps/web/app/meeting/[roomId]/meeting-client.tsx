...
export default function MeetingClient({ roomId }: { roomId: string }) {
  ...
  useEffect(() => {
    ...
    // 1. If running on localhost (browser), try localhost:3001 first.
    // 2. Otherwise use env var or Cloudflare fallback.
    let websocketUrl = process.env.NEXT_PUBLIC_WEBSOCKET_URL || 'https://icn05-c01.trycloudflare.com'; // Update this dynamically if possible, or instruct user
    // Wait, the log output shows `ip=198.41.200.53 location=icn05 protocol=quic` but NOT the URL yet.
    // The previous `type tunnel.log` output was truncated or didn't show the URL line.
    // I need to check the log content again carefully.
    // The log output in step 9278 was:
    // 2025-11-30T05:21:23Z INF Starting metrics server on 127.0.0.1:20241/metrics 
    // 2025-11-30T05:21:23Z INF Registered tunnel connection connIndex=0 connection=ba95d5da-9d2d-46a2-810b-e10c672ffb4f event=0 ip=198.41.200.53 location=icn05 protocol=quic
